...
class dincelcrm_mail_thread(osv.AbstractModel):
...
class dincelcrm_mail(osv.TransientModel):
	...
	def default_get(self, cr, uid, fields, context=None):
		#res = super(dincelcrm_mail, self).default_get(cr, uid, ids, vals, context=context)
		if context is None:
			context = {}
		result = super(dincelcrm_mail, self).default_get(cr, uid, fields, context=context)
	...
	def update_flag_model(self, cr, uid, values, context=None):
		#_logger.error("_update_flag_model111["+str(context.get('default_model'))+"]["+str(context.get('mark_need_approval'))+"]["+str(context.get('default_need_approval'))+"]")
		if context.get('default_model') == 'account.analytic.account' and context.get('mark_need_approval'):
			if context.get('default_need_approval'):
				_id  = context.get('default_res_id')
				_obj = self.pool.get('account.analytic.account').browse(cr, uid, _id, context=context)
				if _obj.state != 'need_approval':
		...
		if context.get('default_model') == 'account.analytic.account' and context.get('mark_as_approved'):			
			if context.get('mark_as_approved'):
				_id  = context.get('default_res_id')
				_obj = self.pool.get('account.analytic.account').browse(cr, uid, _id, context=context)
				if _obj.state != 'approved':
			...
			'''
	def send_mail_for_approval(self, cr, uid, ids, context=None):
		#_logger.error("send_mail_for_approval[Mail send function...]["+str(ids)+"]")
		...
		#_logger.error("send_mail_for_approval111[Mail send function...]["+str(ids)+"][MSG ID : "+str(msg_id)+"]")
		if msg_id:
			self.update_flag_model(cr, uid, ids, context=context)	
	...
	def send_mail_dcs(self, cr, uid, ids, context=None):
		...
		_logger.error("_send_mail_dcs[Mail send function...]["+str(ids)+"]")
		context = dict(context or {})
		context.pop('default_email_to', None)
		context.pop('default_partner_ids', None)
		msg_id = False
