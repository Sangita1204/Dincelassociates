...
class dincelaccount_sale_order(osv.Model):
	...
	_columns={
		...
		'x_cr_limit_over': fields.function(cr_limit_over, method=True, string='Cr Limit', type='boolean'),
		'x_create_deposit': fields.function(create_deposit_chk, method=True, string='create_deposit_chk', type='boolean'),
	...
	def write(self, cr, uid, ids, vals, context=None):
		...
		for record in self.browse(cr, uid, ids):
			...
			id			= record.id	
			has_custom	= record.x_has_custom	
		#_logger.error("dincelaccount_sale_order.has_custom["+str(has_custom)+"]["+str(id)+"]")	 
		if has_custom and has_custom == '0':
			try:	
				sql="update sale_order set x_deposit_exmpt=True where id=%s " % (id)
				cr.execute(sql)
				#_logger.error("dincelaccount_sale_order.sql["+str(sql)+"]")	 
			except ValueError:
				has_custom=None
	...
	def updatelink_order_dcs(self, cr, uid, ids, context=None):
		...
		item = str1['item']
		_logger.error("updatelink_order_dcs.updatelink_order_dcs["+str(item)+"]["+str(item['post_status'])+"]")
