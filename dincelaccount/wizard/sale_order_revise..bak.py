		...
		for record in self.browse(cr, uid, ids, context=context):	#record = self.browse(cr, uid, ids[0], context=context)
			if record.order_id:
				...
				vals = {
					...
					'name':_new_name,
					'x_colorcode': o.x_colorcode,
					'user_id':o.user_id.id,
					'x_pudel':o.x_pudel,
					'x_revision':True,
					'x_revise_sn': revise_sn,
					'x_revise_type': record.revise_type,
				...
				if o.x_street:
					vals['x_street']=o.x_street
				if o.x_postcode:
					vals['x_postcode']=o.x_postcode
				if o.x_suburb:
					vals['x_suburb']=o.x_suburb		
				if o.x_dt_request:
					vals['x_dt_request']=o.x_dt_request
				if o.x_quote_id:
					vals['x_quote_id']=o.x_quote_id.id
				if o.payment_term:
					vals['payment_term']=o.payment_term.id
				if o.origin:
					vals['origin']=o.origin
				if o.x_project_id:
					vals['x_project_id']=o.x_project_id.id
				if o.x_contact_id:
					vals['x_contact_id']=o.x_contact_id.id
				if o.x_warehouse_id:
					vals['x_warehouse_id']=o.x_warehouse_id.id
				if o.x_state_id:
					vals['x_state_id']=o.x_state_id.id
				if o.x_country_id:
					vals['x_country_id']=o.x_country_id.id
				if o.company_id:
				...
				_id = self.pool.get('sale.order').create(cr, uid, vals, context=context)
				if _id:
					#_lines_new = []		
					for line in record.new_lines:
						vals1 = {
							...
							'order_id':_id,
							'x_region_id':line.region_id.id,
							'x_order_length':line.order_length,
						...
						_idsub = self.pool.get('sale.order.line').create(cr, uid, vals1, context=context)
					#---------------------------------------------------------------------------------------------------------------------------
