...
class dincelaccount_move(osv.Model):
	...
	def button_duplicate(self, cr, uid, ids, context=None):
		...
		for record in self.browse(cr, uid, ids, context=context):
			...
			date_post = fields.date.context_today(self,cr,uid,context=context)
			_objperiodcr	= _operiod.find(cr, uid, date_post, context=context)#[0]
			if _objperiodcr:
				period_id 			= _objperiodcr[0] #Period => selected index of period object
			...
			vals = {
				...
				'ref': record.ref,
				'to_check': record.to_check,
				'balance': record.amount,
				'state': 'draft',
				'name': '/',
				'x_region_id': record.x_region_id.id,
				'x_coststate_id': record.x_coststate_id.id,
			}
			move_id = move_obj.create(cr, uid, vals, context=context)
			_sn=len(record.line_id)
			if(_sn > 0):
				for line in record.line_id:
					vals1 ={
						...
						'account_tax_id':line.account_tax_id,
						'tax_amount':line.tax_amount,
						'tax_code_id':line.tax_code_id,
						#'reconcile_partial_id':line.reconcile_partial_id.id,
					}
					#_logger.error("button_duplicate_222["+str(vals)+"]["+str(vals1)+"]")
					line_id = move_line_obj.create(cr, uid, vals1, context=context)
			
		view_id 		= self.pool.get('ir.ui.view').search(cr, uid, [('name', '=', 'account.move.form')], limit=1) 	#redirect to sale order form view
		value = {
			'domain': str([('id', 'in', move_id)]),
			'view_type': 'form',
			'view_mode': 'form',
...
class dincelaccount_journal(osv.Model):
	_inherit = "account.journal" 
	_columns = {
		'x_purchase':fields.boolean('Purchase'),
		'x_sale':fields.boolean('Sale'),
		'x_default':fields.boolean("Default in Sale/Purchase"),
		'x_paymethod_id':fields.many2one('dincelaccount.paymethod', 'Default Pay Method'),
	}
	_defaults = {
...
class dincelaccount_journal_dcs(osv.Model):
	...
	def stock_inventory_journal(self, cr, uid, ids, inv_id, context=None):
		_oinv 		= self.pool.get('stock.inventory')
		_operiod 	= self.pool.get('account.period') 
		_oprod 		= self.pool.get('product.product')
		_oglcodes 	= self.pool.get('dincelaccount.config.settings')
		_obomline 	= self.pool.get('mrp.bom.line')
		_ojournal 	= self.pool.get('account.journal')
		_obj  		= _oinv.browse(cr, uid, inv_id, context=context)
		...
		_journal_id		= _ojournal.search(cr, uid, [('code', '=','STJ')], limit=1)[0]
		
		#5-5200 Stock adjustment
		acct_stk_adj=_oglcodes.stock_take_variance.id 
		#1-1310 Raw Materials Stock at hand
		acct_stk_hand=_oglcodes.stock_inventory.id
		#1-1301 Finished goods Stock on Hand
		...
		_tot_rm=0.0	
		_totamt_rm=0.0
		date_post = _obj.x_date
		
		for _line in _obj.line_ids:	
			_qty=	_line.x_qty_adjust
			_amt=	_line.x_unit_cost
			_amt1 = float(_qty)*float(_amt)
			_id			= _obomline.search(cr, uid, [('product_id', '=',_line.product_id.id)], limit=1) #(cr, uid, [()], limit=1) 	
			if _id:
				#raw material
				_totamt_rm	+=_amt1
				_tot_rm		+=_qty
		...
		if _objperiodcr:
			...
			if _id:
				...
				if abs(_tot_fg)>0.0:		
					...
					if _tot_fg>0.0:
						self.debit			= abs(_totamt_fg)
						self.credit			= 0
					else:
						self.debit			= 0
						self.credit			= abs(_totamt_fg)
					self.insert_move_line(cr, uid, ids, context=context)
					
					account_id=acct_stk_adj
					self.account_id		= account_id
				...
				if abs(_tot_rm)>0.0:
					account_id=acct_stk_hand
					self.account_id		= account_id
					if _tot_rm>0.0:
						self.debit			= abs(_totamt_rm)
						self.credit			= 0
					else:
						self.debit			= 0
						self.credit			= abs(_totamt_rm)
					self.insert_move_line(cr, uid, ids, context=context)
	...
	def mo_produced_qty_journal_dcs(self, cr, uid, ids, mo_id, qty, context=None):
		...
		if _obj.bom_id:#always have bom id....
			for line in _obj.bom_id.bom_line_ids:
				_price    = line.product_id.standard_price
				#_lineqty  = line.product_qty
				#_qty_made = line.product_qty  * qty
				_qty_made = round((line.product_qty  * qty),4) #BOM qty x produce qty 
				#if order_len>0:
			...
			#always have bom id....but just in case does not have....
			...
			for line in _obj.product_lines:
				...
				if ar_items_qty.has_key(line.product_id.id) == False:
					...
					ar_items_qty[line.product_id.id] += _qty_made	
			#for partial movement...some reasons the move_lines are not reserved when logic cursor comes here....
			for line in _obj.move_lines:
				...
				cost_price	= round((_price*_qty_made),4) #4 digit accuracy for price variance in journal....
				if line.product_id.x_bom_cat in ['labour','overheads']:
					cost_overhead += cost_price
				else:
					cost_material += cost_price
		
		#_logger.error("mo_produced_qty_journal_111[%s] product_lines[%s] move_lines[%s] " % (ar_items_qty,_obj.product_lines, _obj.move_lines))
		
		origin 		= _obj.name
		journal_ref = "MOJ" 
		move_name 	= journal_ref + "_" + str(origin)
		
		#_logger.error("mo_produced_qty_journal_dcs move_name[%s] _qty_made[%s] tot_qty[%s] [%s]qty[%s]" % (move_name,_qty_made, tot_qty, cost_material, qty))
		
		if _obj.x_sale_order_id:
			_partner_id=_obj.x_sale_order_id.partner_id.id 
		else:
			_partner_id=None
		...
		if _objperiodcr:
			...
			if _id:
				#---------------------1---------------------
				cost_material	= round(cost_material, 2)	#2 digits are saved in database....so round 2
				cost_overhead	= round(cost_overhead, 2) 	#2 digits are saved in database....so round 2
				amt_fgsh		= round((cost_material+cost_overhead), 2)	#2 digits are saved in database....so round 2
				#cost_overhead	= round((amt_fgsh-cost_material), 2)		#2 digits are saved in database....so round 2
				state_line		= "valid"
				account_id		= acct_stock_fgoods
				debit			= amt_fgsh
				credit 			= 0
	...
	def mo_produce_start_journal_dcs(self, cr, uid, ids, mo_id, context=None):
		...
		if _objperiodcr:
			...
			state_move			= "posted"	#"draft" [NOTE, if posted then it will automatically reported in Tiral Balance, General Ledger, reports, etc]
			amt_wip_total=round(amt_wip_total,2)
			self.ref_name 		= origin+"_"+str(mo_id)
			self.state_move 	= state_move
			self.journal_id 	= _journal_id
			self.move_name 		= move_name
...
class dincelaccount_journal(osv.Model):
					...
					for line in _obj.picking_line:
						...
						if line.product_id.x_prod_cat == "freight":
							...
							if length > 0 and line.product_id.x_is_main == '1':
								if line.product_id.x_dcs_itemcode and "P-3" in line.product_id.x_dcs_itemcode:
									qty=round((length*qty*0.001/3),4) #equavilent m2 value for sales...
									#qty_lm = each (already assigned above) so no change....
								else:
									qty_lm=length*qty*0.001 #for cogs..........
							...
							#else:
							#	#qty 			= line.ship_qty 
							amt_item	= round((price_unit*qty),4) #for unrealised sales ...and product sales value....
							sale_amt += amt_item
	...
	def invoice_refund_bank_journal(self, cr, uid, ids,_id, journal_id, account_id,_dt, context=None):
		_objperiod 	= self.pool.get('account.period') 
		_obj     = self.pool.get('account.invoice').browse(cr, uid, _id, context=context)
		tot_paid = 0.0
		tot_fee  = 0.0
		tot_tot  = 0.0	
		if _obj.x_balance_amt: 
			_obj_journ     = self.pool.get('account.journal').browse(cr, uid, journal_id, context=context)
			self._get_config_data(cr, uid, ids, context)
			
			self.journal_id = 	journal_id
			amount_bal=abs(_obj.x_balance_amt)
			
			if amount_bal != 0.0 and self.journal_id:	
				 
				#self.invoice_type 	= _obj.type
				#self.journal_id  	= self.journal.id
				journal_ref 		= _obj_journ.code
				_suffix="" #future use incase
				if _obj.number:
					_ref=" ["+str(_obj.number)+"]"
				else:
					_ref=""
				
				self.name 		= journal_ref + "/refund/" + str(_obj.id)
				if _obj.reference:
					_ref 	+=  "/" + str(_obj.reference)
				...
				self.date		= _dt#_obj.date_invoice
				...
				if self.move_id:
					...
					debit		=0
					credit		=amount_bal
					self.add_journal_moveline(cr, uid, ids, debit, credit, account_id2, context)

					account_id2	=_obj.account_id.id
					debit		=amount_bal
					credit		=0
					self.add_journal_moveline(cr, uid, ids, debit, credit, account_id2, context)
				return True
		...
		if _obj.x_payline_ids: 
			...
			if tot_paid and self.journal:	
				...
				if self.move_id:
					...
					if _obj.x_amount_xtra and _obj.x_paymethod_id:
						
						account_id	= _obj.x_paymethod_id.account_dr_id.id
						if tot_paid>0:
							debit		= _obj.x_amount_xtra
							credit		= 0
						else:
							debit		= 0
							credit		= abs(_obj.x_amount_xtra)

						self.add_journal_moveline(cr, uid, ids, debit, credit, account_id, context)
						
						tot_paid	+= _obj.x_amount_xtra
				...
				return self.move_id
	#def supplierpayment2journals(self, cr, uid, ids,_id, context=None):				
	#	return self._supplierpayment_journals
	def supplierpayment2journalsxx(self, cr, uid, ids,_id, context=None):
		_objperiod 	= self.pool.get('account.period') 
		_obj     = self.pool.get('account.voucher').browse(cr, uid, _id, context=context)
		tot_paid = 0.0
		#tot_fee  = 0.0
	...
	def payment2journals(self, cr, uid, ids,_id, context=None):
		return self._payment2journals(cr, uid, ids,_id, "normal", context=context)
		'''_objperiod 	= self.pool.get('account.period') 
		_obj     = self.pool.get('account.voucher').browse(cr, uid, _id, context=context)
		tot_paid = 0.0
		tot_fee  = 0.0
	...
	def _payment2journals(self, cr, uid, ids,_id, _type,context=None):
		_objperiod 	= self.pool.get('account.period') 
		_obj     = self.pool.get('account.voucher').browse(cr, uid, _id, context=context)
		tot_paid = 0.0
		tot_fee  = 0.0
		...
		if _obj.x_payline_ids: 
			...
			if tot_tot > 0 and self.journal:	
				...
				if _obj.number:
					...
					_ref=""
				if _type=="reverse":
					self.name 		= journal_ref + "_reverse_" + str(_obj.id) + _suffix
				else:
					self.name 		= journal_ref + "_" + str(_obj.id)
				if _obj.reference:
					_ref 	+=  " " + str(_obj.reference)
				...
				self.period_id	= _objperiod.find(cr, uid, _obj.date, context=context)[0]
				state_move		= "posted"	#"draft" posted [NOTE, if posted then it will automatically reported in Tiral Balance, General Ledger, reports, etc]
				obj_move 		= self.pool.get('account.move')
		...
		return False	
	def payment2journals_reverse(self, cr, uid, ids,_id, context=None):
		return self._payment2journals(cr, uid, ids,_id, "reverse", context=context)
	...
	def _invoice2journals(self, cr, uid, ids,new_id, context=None):
		...
		if self.journal:#_obj.journal_id:
			...
			#see ...->add_journal_moveline() below....
			#-------------------------------------------------------------------------
			self.invoice_type 	= 'na'# _obj.type do not revrse 2/4/2017 [
			#-------------------------------------------------------------------------
			
			ref = "inv/%s/%s " % (new_id, _obj.number)
			#in order to negate the negative value in credit notes...
			#remove -ve and take abs.....value...
			#if self.invoice_type in ["out_refund","in_refund"]: 
			#	x_factor=-1.0 #eg credit note are converted into -ve...value
			#else:
			#	x_factor=1.0
			...
			for _line in _obj.invoice_line:
				...
				found 		= False	
				if prod_cat == "deposit" or  prod_cat == "balance":
					#cause no cost price for deposit
					costprice	= 0
					deposit_amt	+= price
			...
			if self.journal_id:
				...
				if self.move_id:
					_net_gst_amt=0.0	
					for _gst in _obj.tax_line:
						_acct_id=_gst.account_id.id
						_amt=_gst.amount
						_net_gst_amt+=_amt
						if abs(_amt)>0.0:
							if _amt<0.0:
					...
					if abs(deposit_amt)>0.0:
						if deposit_amt<0.0:
							debit		=abs(deposit_amt)
							credit		=0
						else:
							debit		=0
							credit		=deposit_amt
						...
						self.add_journal_moveline(cr, uid, ids,debit, credit,account_id, context)	
					if abs(unreal_disc_amt)> 0.0:#001
						if unreal_disc_amt<0.0:
							debit		=0
							credit		=abs(unreal_disc_amt)
						else:
							debit		=unreal_disc_amt
							credit		=0
						if _obj.x_inv_type and _obj.x_inv_type=="refundreturn":
							account_id	=self.acct_sale_discount
						else:
							account_id	=self.acct_unrealised_discount
						self.add_journal_moveline(cr, uid, ids,debit, credit,account_id, context)	
					
					if abs(unreal_deli_amt)> 0.0:#unreal_deli_amt > 0.0001:
						if unreal_deli_amt<0.0:
							debit		=abs(unreal_deli_amt)
							credit		=0
						else:
							debit		=0
					...
					sale_unreal_amt=sale_unreal_amt+unreal_disc_amt
					if abs(sale_unreal_amt) > 0.0:#if sale_unreal_amt > 0.0001:
						if sale_unreal_amt<0.0:
							debit		=abs(sale_unreal_amt)
							credit		=0
						else:
							debit		=0
							credit		=sale_unreal_amt
						if _obj.x_inv_type and _obj.x_inv_type=="refundreturn":
							account_id	=self.acct_sale
						else:
							account_id	=self.acct_sale_unrealised
					...
					#_logger.error("invoice_sales_validate:tax_deposit_amt.deposit_amt["+str(sale_amt)+"]["+str(deposit_amt)+"]["+str(_net_gst_amt)+"]")
					
					sale_amt=sale_amt+_net_gst_amt+deposit_amt
					...
					if abs(sale_amt) > 0.0:
						...
						account_id	=self.acct_trade_dr
						self.add_journal_moveline(cr, uid, ids,debit, credit,account_id, context)
					'''total_tax_amt=	total_tax_amt+tax_deposit_amt
					if total_tax_amt != 0.0 and self.sale_tax_id:
						if total_tax_amt<0.0:
							debit		=abs(total_tax_amt)
							credit		=0
						else:
							debit		=0
					...
					#deposit invoice...special condition
					if tax_deposit_amt!=0.0:
						if tax_deposit_amt<0.0:
						...
						account_id	=self.sale_tax_id
						self.add_journal_moveline(cr, uid, ids,debit, credit,account_id, context)
					deposit_amt_tot=deposit_amt+tax_deposit_amt
					if deposit_amt_tot!=0.0:
						if deposit_amt_tot<0.0:
							debit		=0
							credit		=abs(deposit_amt_tot)
						else:
							debit		=deposit_amt_tot
							credit		=0
						account_id	=self.acct_trade_dr
						self.add_journal_moveline(cr, uid, ids,debit, credit,account_id, context)
					if deposit_amt!=0.0:
