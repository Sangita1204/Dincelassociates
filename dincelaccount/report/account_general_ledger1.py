...
class general_ledger1(report_sxw.rml_parse, common_report_header1):
	_name = 'account.report.general.ledger1'
	_inherit = "account.report.general.ledger"
	def set_context_dcs(self, objects, data, ids, report_type=None):
		new_ids = ids
		...
		if data['coststate_id']:
			ctx['coststate_id'] = data['coststate_id']
			
		if data['region_id']:
			ctx['region_id'] = data['region_id']
		else:
			ctx['region_id'] = None
		...
		return super(general_ledger1, self).set_context_dcs(objects, data, new_ids, report_type=report_type)
	...
	def __init__(self, cr, uid, name, context=None):
		...
		self.localcontext.update( {
			'time': time,
			'lines': self.lines_dcs,
			'sum_debit_account': self._sum_debit_account_dcs,
			'sum_credit_account': self._sum_credit_account_dcs,
			'sum_balance_account': self._sum_balance_account_dcs,
			'sum_currency_amount_account': self._sum_currency_amount_account_dcs,
	...
	def _sum_currency_amount_account_dcs(self, account):
	...
	def _sum_debit_account_dcs(self, account):
	...
	def _sum_credit_account_dcs(self, account):
		if account.type == 'view':
	...
	def _sum_balance_account_dcs(self, account):
		if account.type == 'view':
	...
	def _get_account_dcs(self, data):
		if data['model'] == 'account.account':
			return self.pool.get('account.account').browse(self.cr, self.uid, data['form']['id']).company_id.name
		return super(general_ledger1 ,self)._get_account_dcs(data)
...
class report_generalledger_dcs(osv.AbstractModel):
	_name = 'report.account.report_generalledger_dcs'
	_inherit = 'report.abstract_report'
	_template = 'dincelaccount.report_generalledger_dcs'
	_wrapped_report_class = general_ledger1
