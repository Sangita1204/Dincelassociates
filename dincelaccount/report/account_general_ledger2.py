...
class account_gledger2(report_sxw.rml_parse, common_report_header1):
	...
	def set_context(self, objects, data, ids, report_type=None):
		...
		if data['form']['region_id']:
			ctx['region_id'] = data['form']['region_id']
		else:
			ctx['region_id'] = None
		if data['form']['coststate_id']:
			ctx['coststate_id'] = data['form']['coststate_id']	
	...
	def get_children_accounts(self, account):
		...
		#_logger.error("get_children_accountsget_query" + str(self.query))
		...
		for child_account in self.pool.get('account.account').browse(self.cr, self.uid, ids_acc, context=self.context):
			...
			""" % (self.query)
			#if self.context['region_id']:
			#	sql = sql +" AND l.x_region_id="+ str(self.context['region_id'][0])
	...
	def lines(self, account):
		...
		self.tot_lines += len(res_lines)
		#_logger.error("tot_linestot_linesr_items_rem["+str(self.tot_lines)+"]")
		res_init = []
