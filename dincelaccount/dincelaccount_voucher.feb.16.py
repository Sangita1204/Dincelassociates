...
class dincelaccount_act_voucher(osv.Model):
			...
			for line in _obj.x_payline_ids:
				...
				if line.amount > 0 and line.paymethod_id.id==False:
					...
					if line.invoice_id.x_sale_order_id: #check for full/partial paid status
						self.pool.get('sale.order').update_payment_order(cr, uid, ids, line.invoice_id.x_sale_order_id.id, context=context)		
	...
	def onchange_partner_id_new(self, cr, uid, ids, partner_id, journal_id, amount, currency_id, ttype, date, context=None):
		...
		#state=open
		_ids = _inv.search(cr, uid, [('partner_id', '=', partner_id),('state','=','open'),('type','=','out_invoice')],order="id asc")
