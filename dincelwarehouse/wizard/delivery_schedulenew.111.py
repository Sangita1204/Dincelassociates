		...
		if not err_found:
			...
			for line in record.reserve_lines:
				...
				if myDict.has_key(_key):
					vals= myDict[_key]
					vals['qty_order']=	float(vals['qty_order'])+_qtyorder2
					...
					vals['qty_remain']=	float(vals['qty_remain'])-_qty+_qtyorder2
				else:
		...
		if context and active_id:
			...
			for line in _obj.order_line:
				if line.product_id.x_prod_cat not in['freight','deposit','service']  and line.product_id.type !="service":
					...
					#else:
					qty_reserve=0
					sql="""select sum(r.reserve_qty) from dincelmrp_production_reserve r,sale_order o where 
							r.order_id=o.id and r.order_id='%s' and r.product_id='%s'  
					...
					if rows and len(rows) > 0 and rows[0]:	
						qty_mrp= int(rows[0])
					if qty_mrp==0 and _reserved==False:#if not found in resserver of MRP then default mrp
						qty_mrp=line.x_order_qty
						
					'''for mrp in _obj.x_mrp_lines_dcs: #all MRP items (MRP-1, MRP-2, etc)
						for _item in mrp.reserve_line:
							if _item.product_id.id == line.product_id.id and _item.order_length==line.x_order_length:
								...
								if _item.location_id and _found==False:
									_location=_item.location_id.id 
									break'''
					_qty_delivered 	= _prod.qty_delivered_net_v2(cr, uid, ids, line.product_id.id, order_length, _obj.id)		
					if _reserved and qty_reserve > 0:
						_qtyonhand_res  = _prod.qty_stock_reserved_new( cr, uid, ids,line.product_id.id,order_length, _obj.id) 
						#_qtyonhand = _qtyonhand - _qty_delivered
					#else:
					#	#if _found:
					#	#	_qtyonhand 	= oprod.stock_value(cr, uid, line.product_id.id,int(line.x_order_length), _location, context=context)
					#	#else:
					#	#	_qtyonhand 	= _prod.qty_produced_net(cr, uid, ids, line.product_id.id, order_length, _obj.id)				
					#	_qtyonhand 	= _prod.qty_produced_net(cr, uid, ids, line.product_id.id, order_length, _obj.id)				
					#-------------------------------------------------------------------------------------------------------
					_qtyonhand_mrp 	= _prod.qty_produced_net(cr, uid, ids, line.product_id.id, order_length, _obj.id)				
					...
					#if line items are duplicate (one length one product MRP/reserve seperated...) then duplicate record here...
					#so assume the line item is alwayws unique ...product_id.length
					qty_res_picked=0	
					if qty_reserve>0 and qty_mrp>0:#qty is partially reserved and produced partially ...
						...
						if rows and len(rows) > 0 and rows[0]:
							...
							_qty_delivered=_qty_delivered-qty_res_picked
					else:
						qty_res_picked=_qty_delivered
						#_qtyonhand_res=_qtyonhand
						#_qtyonhand=_qty_produced
					if _reserved and qty_reserve>0: 
						valres=vals1.copy()
						valres['qty_onhand']= _qtyonhand_res	
					...
					if qty_mrp>0:
						valmrp=vals1.copy()
						valmrp['qty_onhand']= _qtyonhand_mrp	 
