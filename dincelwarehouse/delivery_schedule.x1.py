...
class dincelwarehouse_delivery_docket(osv.osv_memory):
	...
	_columns = {
		'date': fields.date('Date Entry'),
		'date_actual': fields.date('Date Actual'),
	...
	def on_change_qty(self, cr, uid, ids, _qty, _lines, context=None):
		...
		if context and active_id:
			...
			if _objDeli.pending_invoice==True:
				_objDeli.authorize_docket!=True:
					raise osv.except_osv(_('Error!'), _('Pending COD invoice/s found, please contact account team to continue !!'))
