...
class stock_value_report1(report_sxw.rml_parse):
	...
	def _get_details(self,context=None):
		...
		for row in ret_res:
			...
			sql="""SELECT sum(sl.ship_qty) as prod_qty, sl.product_id as prod_id, st.name, st.x_dcs_group, st.x_dcs_itemcode
			...
			WHERE st.active='t' and st.x_dcs_itemcode like '"""+str(prod_like)+"""%' AND st.x_prod_type='acs'"""
